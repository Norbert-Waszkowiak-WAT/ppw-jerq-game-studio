...
public class abilitiesHandler : NetworkBehaviour
{
    ...
    {
        LogWriter.WriteLog("send wall raising to all players");
    ...
    {
        LogWriter.WriteLog("finish wall raising client rpc");
    ...
    {
        LogWriter.WriteLog("finish wall raising server rpc");
    ...
    void FinishWallRaising(Vector3 position, Quaternion rotation)
    {
        ...
        LogWriter.WriteLog("finish wall raising");
    ...
    {
        LogWriter.WriteLog("start wall raising");
        ...
        else
        {
            ...
            usedAbility = null;
            wallBlueprint.active = false;
            LogWriter.WriteLog("end wall raising");
        }
    }
    ...
    {
        LogWriter.WriteLog("super speed");
        ...
        LogWriter.WriteLog("end super speed");
        inSuperSpeed = false;
        playerMovementInstance.sprintSpeed /= (1 + speedIncrease / 100);
    }
    ...
    void Flash(Ability ability)
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, distance, layerMask) && hit.transform != null)
        {
            if (hit.distance < 1)
            {
                ...
                LogWriter.WriteLog("distance < 1");
                return;
            }
            else
            {
                Debug.Log("start position: " + transform.position + "end position: " + (hit.point - transform.forward));
                LogWriter.WriteLog("start position: " + transform.position + "end position: " + (hit.point - transform.forward));
        ...
        else
        {
            ...
            LogWriter.WriteLog("start position: " + transform.position + "end position: " + (transform.position + transform.forward * distance));
            rb.MovePosition(transform.position + transform.forward * distance);
        }
        Debug.LogError("flash");
        LogWriter.WriteLog("flash");
    }
